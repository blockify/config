...
// Fallbacks for other operating systems
    ...
    paths.config = paths.config || path.join(base, '.config/block');
    paths.data = paths.data || path.join(base, '.local/share/block');
    paths.cache = paths.cache || path.join(base, '.cache/block');
}

paths.tmp = path.resolve(path.join(tmp, 'block'));
